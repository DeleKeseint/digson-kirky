...
static wolfson_config_t wolfson = {0, 0, 0, 0, 50, "Normal"};
...
static apo_config_t apo_config = {0, 50};
...
static void get_real_bluetooth_status(void) {
    ...
    if (cmd_exists("bluetoothctl")) {
        ...
        if (fp) {
            char line[256];
            if (fgets(line, sizeof(line), fp)) {
                if (strstr(line, "yes")) settings.bluetooth_enabled = 1;
            }
            pclose(fp);
        }
    }
    ...
    if (getifaddrs(&ifaddr) == 0) {
        for (struct ifaddrs *ifa = ifaddr; ifa; ifa = ifa->ifa_next) {
            if (ifa->ifa_name && (strncmp(ifa->ifa_name, "rmnet", 5) == 0 ||
                ...
                if (ifa->ifa_flags & IFF_UP) settings.mobile_data_enabled = 1;
            }
        }
        freeifaddrs(ifaddr);
    }
    ...
    if (getifaddrs(&ifaddr) == 0) {
        for (struct ifaddrs *ifa = ifaddr; ifa; ifa = ifa->ifa_next) {
            if (ifa->ifa_name && strstr(ifa->ifa_name, "ap")) {
                if (ifa->ifa_flags & IFF_UP) settings.hotspot_enabled = 1;
            }
        }
        freeifaddrs(ifaddr);
    }
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp) && dns_count < 2) {
            ...
            if (sscanf(line, "nameserver %63s", ns) == 1) {
                if (dns_count == 0) strncpy(settings.dns1, ns, sizeof(settings.dns1) - 1);
                else strncpy(settings.dns2, ns, sizeof(settings.dns2) - 1);
                dns_count++;
            }
        }
        fclose(fp);
...
static void get_flight_mode_status(void) {
    ...
    if (fp) {
        ...
        if (fgets(status, sizeof(status), fp)) {
            if (atoi(status) == 1) settings.flight_mode = 1;
        }
        pclose(fp);
    }
}
...
static void get_nfc_status(void) {
    ...
    if (fp) {
        char status[32];
        if (fgets(status, sizeof(status), fp)) {
            if (atoi(status) == 1) settings.nfc = 1;
        }
        pclose(fp);
    }
}
...
static int has_wm8994(void) {
    ...
    if (access("/dev/snd/pcmC0D0p", F_OK) == 0 && access("/dev/snd/pcmC0D0c", F_OK) == 0) {
        return 1;
    }
    return 0;
}
...
static void save_wolfson_config(void) {
    int use_termux_path = cmd_exists("pkg");
    ...
    if (use_termux_path) {
        ...
        snprintf(mkdir_cmd, sizeof(mkdir_cmd), "mkdir -p %s", path);
        system(mkdir_cmd);
        snprintf(path, sizeof(path), "/data/data/com.termux/files/usr/etc/digson/get/WolfsonConfig");
    } else {
        snprintf(path, sizeof(path), "/etc/WolfsonConfig");
    }
    
    FILE *fp = fopen(path, "w");
    if (!fp) {
        fprintf(stderr, "Cannot save Wolfson config to %s\n", path);
        return;
    }
    
    fprintf(fp, "# Wolfson WM8994 Configuration for get command\n");
    fprintf(fp, "USE_FOR_GET=%d\n", wolfson.used_for_get);
    fprintf(fp, "NETWORK_DISCONNECTED=%d\n", wolfson.network_disconnected);
    fprintf(fp, "FIND_ROUTER=%d\n", wolfson.find_router);
    fprintf(fp, "USER_CONFIRMED_NO_CARD=%d\n", wolfson.user_confirmed_no_card);
    fprintf(fp, "VOLUME=%d\n", wolfson.volume);
    fprintf(fp, "EQ=%s\n", wolfson.eq_preset);
    
    fclose(fp);
    printf("Wolfson config saved to: %s\n", path);
}

static void load_wolfson_config(void) {
    char *paths[] = {
        "/data/data/com.termux/files/usr/etc/digson/get/WolfsonConfig",
        "/etc/WolfsonConfig",
        NULL
    };
    ...
    for (int i = 0; paths[i]; i++) {
        ...
        if (fp) {
            ...
            while (fgets(line, sizeof(line), fp)) {
                ...
                int val;
                if (sscanf(line, "USE_FOR_GET=%d", &val) == 1) {
                    wolfson.used_for_get = val;
                } else if (sscanf(line, "NETWORK_DISCONNECTED=%d", &val) == 1) {
                    wolfson.network_disconnected = val;
                } else if (sscanf(line, "FIND_ROUTER=%d", &val) == 1) {
                    wolfson.find_router = val;
                } else if (sscanf(line, "USER_CONFIRMED_NO_CARD=%d", &val) == 1) {
                    wolfson.user_confirmed_no_card = val;
                } else if (sscanf(line, "VOLUME=%d", &val) == 1) {
                    wolfson.volume = val;
                    if (wolfson.volume < 0) wolfson.volume = 0;
                    if (wolfson.volume > 100) wolfson.volume = 100;
                } else if (sscanf(line, "EQ=%31s", wolfson.eq_preset) == 1) {
                    // EQ preset loaded
                }
            }
            fclose(fp);
...
static void show_wolfson_blue_screen(void) {
    load_wolfson_config();
    ...
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(1, 2, "Wolfson WM8994 Audio Processor");
    attroff(A_BOLD);
    ...
    do {
        ...
        mvprintw(2, 2, "Device: Samsung Galaxy S (GT-I9000)");
        ...
        for (int i = 0; i < 6; i++) {
            ...
            const char *status = "";
            switch (i) {
                case 0: status = wolfson.used_for_get ? "[open]" : "[close]"; break;
                case 1: status = wolfson.network_disconnected ? "[open]" : "[close]"; break;
                case 2: status = wolfson.find_router ? "[open]" : "[close]"; break;
                case 3: 
                    char vol_str[32];
                    snprintf(vol_str, sizeof(vol_str), "[%d]", wolfson.volume);
                    status = vol_str;
                    break;
                case 4: status = wolfson.eq_preset; break;
                case 5: status = "[press Enter]"; break;
            }
        ...
        attron(COLOR_PAIR(3));
        mvprintw(16, 2, "Keys: [x]Use for get  [d]Disconnect  [y]Find router");
        mvprintw(17, 2, "      [v]Volume  [e]EQ  [Enter]Save  [q]Quit");
        ...
        switch (ch) {
            case KEY_UP: if (selected > 0) selected--; break;
            case KEY_DOWN: if (selected < 5) selected++; break;
            case 'x':
            case 'X':
                wolfson.used_for_get = !wolfson.used_for_get;
                break;
            case 'd':
            case 'D':
                wolfson.network_disconnected = !wolfson.network_disconnected;
                break;
            case 'y':
            case 'Y':
                wolfson.find_router = !wolfson.find_router;
                break;
            case 'v':
            case 'V':
                if (selected == 3) {
                    echo();
                    curs_set(1);
                    mvprintw(18, 4, "Enter volume (0-100): ");
                    refresh();
                    char vol_input[8];
                    getnstr(vol_input, sizeof(vol_input));
                    int new_vol = atoi(vol_input);
                    if (new_vol >= 0 && new_vol <= 100) wolfson.volume = new_vol;
                    noecho();
                    curs_set(0);
                }
                break;
            ...
            case 'E':
                if (selected == 4) {
                    const char *presets[] = {"Normal", "Bass", "Treble", "Rock", "Classical"};
                    int current = 0;
                    for (int i = 0; i < 5; i++) {
                        if (strcmp(wolfson.eq_preset, presets[i]) == 0) current = i;
                    }
                    current = (current + 1) % 5;
                    strncpy(wolfson.eq_preset, presets[current], sizeof(wolfson.eq_preset));
                }
                break;
            case '\n':
            case KEY_ENTER:
...
static void show_wolfson_red_screen(void) {
    init_pair(1, COLOR_WHITE, COLOR_RED);
    init_pair(2, COLOR_YELLOW, COLOR_RED);
    ...
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(1, 2, "ERROR: No Wolfson WM8994 Detected");
    attroff(A_BOLD);
    ...
    mvprintw(6, 4, "No WM8994 audio chip found on your system.");
    mvprintw(7, 4, "This feature only works on:");
    mvprintw(9, 6, "- Samsung Galaxy S (GT-I9000)");
    mvprintw(10, 6, "- Devices with Wolfson WM8994 chip");
    
    attron(COLOR_PAIR(2));
    mvprintw(13, 4, "Press Ctrl+X to leave");
    mvprintw(14, 4, "(Entering this screen is a BUG if you have WM8994)");
    attroff(COLOR_PAIR(2));
...
static void show_wolfson_menu(void) {
    initscr();
    start_color();
    cbreak();
    noecho();
    ...
    int has_chip = has_wm8994() || is_i9000();
    
    if (has_chip && !wolfson.user_confirmed_no_card) {
        show_wolfson_blue_screen();
    } else {
        show_wolfson_red_screen();
    }
    
    endwin();
}

// ==================== Realtek APO 设置（原有，保持不变）====================

static int has_realtek_apo(void) {
    if (access("/system/lib/librealtekapo.so", F_OK) == 0) return 1;
    if (access("/vendor/lib/librealtekapo.so", F_OK) == 0) return 1;
    if (access("/system/etc/realtek_apo.conf", F_OK) == 0) return 1;
    
    FILE *fp = popen("getprop ro.product.brand 2>/dev/null", "r");
    if (fp) {
        char brand[64] = {0};
        fgets(brand, sizeof(brand), fp);
        pclose(fp);
        
        if (strstr(brand, "realme")) {
            fp = popen("getprop ro.product.model 2>/dev/null", "r");
            if (fp) {
                char model[64] = {0};
                fgets(model, sizeof(model), fp);
                pclose(fp);
                if (strstr(model, "Q3") || strstr(model, "C11") ||
                    strstr(model, "C12") || strstr(model, "C15") ||
                    strstr(model, "5i") || strstr(model, "6i")) {
                    return 1;
                }
            }
        }
        
        if (strstr(brand, "OPPO") || strstr(brand, "oppo")) {
            fp = popen("getprop ro.product.model 2>/dev/null", "r");
            if (fp) {
                char model[64] = {0};
                fgets(model, sizeof(model), fp);
                pclose(fp);
                if (strstr(model, "A5") || strstr(model, "A7") ||
                    strstr(model, "A9") || strstr(model, "A11") ||
                    strstr(model, "A31") || strstr(model, "A53")) {
                    return 1;
                }
            }
        }
    ...
    if (system("ls /system/lib*/hw/audio.primary.*.so 2>/dev/null | xargs strings 2>/dev/null | grep -i realtek >/dev/null 2>&1") == 0)
        return 1;
    
    return 0;
}

static void save_apo_config(void) {
    int use_termux_path = cmd_exists("pkg");
    
    char path[1024];
    if (use_termux_path) {
        snprintf(path, sizeof(path), "/data/data/com.termux/files/usr/etc/digson");
        char mkdir_cmd[1024];
        snprintf(mkdir_cmd, sizeof(mkdir_cmd), "mkdir -p %s", path);
        system(mkdir_cmd);
        snprintf(path, sizeof(path), "/data/data/com.termux/files/usr/etc/digson/APOConfig");
    } else {
        snprintf(path, sizeof(path), "/etc/APOConfig");
    }
    
    FILE *fp = fopen(path, "w");
    if (!fp) {
        fprintf(stderr, "Cannot save APO config to %s\n", path);
        return;
    }
    
    fprintf(fp, "# Realtek Audio Processing Object Configuration\n");
    fprintf(fp, "VOLUME=%d\n", apo_config.volume);
    fprintf(fp, "ENABLED=%d\n", apo_config.enabled);
    
    fclose(fp);
    printf("APO config saved to: %s\n", path);
}

static void load_apo_config(void) {
    char *paths[] = {
    ...
    for (int i = 0; paths[i]; i++) {
        ...
        if (fp) {
            ...
            while (fgets(line, sizeof(line), fp)) {
                ...
                int val;
                if (sscanf(line, "VOLUME=%d", &val) == 1) {
                    apo_config.volume = val;
                    if (apo_config.volume < 0) apo_config.volume = 0;
                    if (apo_config.volume > 100) apo_config.volume = 100;
                } else if (sscanf(line, "ENABLED=%d", &val) == 1) {
                    apo_config.enabled = val;
                }
            }
            fclose(fp);
            return;
...
static void show_apo_red_screen(void) {
    init_pair(1, COLOR_WHITE, COLOR_RED);
    init_pair(2, COLOR_YELLOW, COLOR_RED);
    ...
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(1, 2, "Realtek Audio Processing Object");
    attroff(A_BOLD);
    
    attron(COLOR_PAIR(2) | A_BLINK);
    mvprintw(4, 4, "!!! COMPONENT NOT FOUND !!!");
    attroff(A_BLINK | COLOR_PAIR(2));
    
    mvprintw(6, 4, "No Realtek APO detected on your system.");
    mvprintw(7, 4, "This feature only works on specific devices:");
    mvprintw(9, 6, "- realme Q3");
    mvprintw(10, 6, "- realme early models (C11, C12, C15, 5i, 6i)");
    mvprintw(11, 6, "- OPPO early A-series (A5, A7, A9, A11, A31, A53)");
    
    attron(COLOR_PAIR(2));
    mvprintw(14, 4, "Press Ctrl+X to leave");
    mvprintw(15, 4, "(Entering this screen is a BUG if you have APO)");
    attroff(COLOR_PAIR(2));
...
static void show_apo_blue_screen(void) {
    load_apo_config();
    ...
    do {
        ...
        mvprintw(4, 4, "Do you have this installed [y](After pressing ctrl + X)");
        if (selected == 0) attroff(COLOR_PAIR(2));
        else attroff(COLOR_PAIR(1));
        
        if (selected == 1) attron(COLOR_PAIR(2));
        else attron(COLOR_PAIR(1));
        mvprintw(6, 4, "[x][After pressing ctrl + y]");
        if (selected == 1) attroff(COLOR_PAIR(2));
        else attroff(COLOR_PAIR(1));
        
        snprintf(volume_str, sizeof(volume_str), "Adjust the volume - %d", apo_config.volume);
        if (selected == 2) attron(COLOR_PAIR(2));
        else attron(COLOR_PAIR(1));
        mvprintw(8, 4, "%s", volume_str);
        if (selected == 2) attroff(COLOR_PAIR(2));
        else attroff(COLOR_PAIR(1));
        
        if (selected == 3) attron(COLOR_PAIR(2));
        else attron(COLOR_PAIR(1));
        mvprintw(10, 4, "save ->");
        if (selected == 3) attroff(COLOR_PAIR(2));
        else attroff(COLOR_PAIR(1));
        
        attron(COLOR_PAIR(3));
        mvprintw(14, 2, "Keys: UP/DOWN=navigate, ENTER=select/adjust");
        mvprintw(15, 2, "      Ctrl+Y=confirm, Ctrl+X=cancel");
        attroff(COLOR_PAIR(3));
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (selected == 2) {
                    attron(COLOR_PAIR(2));
                    mvprintw(12, 4, "Enter volume (0-100): ");
                    echo();
                    curs_set(1);
                    refresh();
...
static void show_realtek_apo_menu(void) {
    initscr();
    start_color();
    cbreak();
    noecho();
    ...
    int has_apo = has_realtek_apo();
    
    if (has_apo) {
        show_apo_blue_screen();
    } else {
        show_apo_red_screen();
    }
