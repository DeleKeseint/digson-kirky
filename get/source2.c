...
int curl_insecure = 0;
static char *output_document = NULL;
static char *output_directory = NULL;
static int continue_download = 0;
static int timestamping = 0;
...
static int tries = 20;
static char *http_user = NULL;
static char *http_passwd = NULL;
static int spider = 0;
static char *post_data = NULL;
static char *header = NULL;
static int no_check_certificate = 0;
static char *bind_address = NULL;
static int limit_rate = 0;

static int curl_progress_bar = 0;
static int curl_fail_silently = 0;
static int curl_include_header = 0;
static int curl_head_only = 0;
static int curl_location = 0;
static int curl_remote_name = 0;
static char *curl_output = NULL;
static int curl_silent = 0;
static int curl_show_error = 0;
static char *curl_upload_file = NULL;
static char *curl_user = NULL;
static char *curl_request = NULL;
static char *curl_data = NULL;
static char *curl_form = NULL;
...
static wolfson_config_t wolfson = {0, 0, 0, 0};
static int wolfson_config_loaded = 0;
...
static void load_realtak_config(void) {
    ...
    if (!fp) {
        if (verbose) fprintf(stderr, "RealtakConfig: No config file found\n");
        return;
    }
    
    if (verbose) fprintf(stderr, "RealtakConfig: Loading from %s\n", used_path);
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int val;
        if (sscanf(p, "USE_FOR_GET=%d", &val) == 1) realtak.used_for_get = val;
        else if (sscanf(p, "NETWORK_DISCONNECTED=%d", &val) == 1) realtak.network_disconnected = val;
        else if (sscanf(p, "FIND_ROUTER=%d", &val) == 1) realtak.find_router = val;
        else if (sscanf(p, "USER_CONFIRMED_NO_CARD=%d", &val) == 1) realtak.user_confirmed_no_card = val;
    }
...
static void load_wolfson_config(void) {
    if (wolfson_config_loaded) return;
    ...
    if (!fp) {
        if (verbose) fprintf(stderr, "WolfsonConfig: No config file found\n");
        return;
    }
    
    if (verbose) fprintf(stderr, "WolfsonConfig: Loading from %s\n", used_path);
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int val;
        if (sscanf(p, "USE_FOR_GET=%d", &val) == 1) wolfson.used_for_get = val;
        else if (sscanf(p, "NETWORK_DISCONNECTED=%d", &val) == 1) wolfson.network_disconnected = val;
        else if (sscanf(p, "FIND_ROUTER=%d", &val) == 1) wolfson.find_router = val;
        else if (sscanf(p, "USER_CONFIRMED_NO_CARD=%d", &val) == 1) wolfson.user_confirmed_no_card = val;
    }
    
    fclose(fp);
    wolfson_config_loaded = 1;
    
    if (verbose) {
        fprintf(stderr, "WolfsonConfig: loaded (used_for_get=%d, network_disconnected=%d, find_router=%d)\n",
                wolfson.used_for_get, wolfson.network_disconnected, wolfson.find_router);
    }
}
...
static int check_realtak_allowed(void) {
    // 检测是否为 i9000 或 WM8994
    if (has_wm8994() || is_i9000()) {
        load_wolfson_config();
        
        if (wolfson.user_confirmed_no_card) return 1;
        
        if (!wolfson.used_for_get) {
            if (!quiet) {
                fprintf(stderr, "get: Wolfson network not configured for use with get command.\n");
                fprintf(stderr, "      Run 'setting' to configure Wolfson settings.\n");
            }
            return 0;
        }
        
        if (wolfson.network_disconnected) {
            if (!quiet) fprintf(stderr, "get: Network disabled by Wolfson configuration\n");
            return 0;
        }
        ...
        if (realtak.user_confirmed_no_card) return 1;
        ...
        if (realtak.network_disconnected) {
            if (!quiet) fprintf(stderr, "get: Network disabled by Realtak configuration\n");
            return 0;
        }
...
static void find_nearby_router(void) {
    int find = (has_wm8994() || is_i9000()) ? wolfson.find_router : realtak.find_router;
    if (!find) return;
    
    if (!quiet) printf("Realtak: Scanning for nearby routers...\n");
...
static int progress_callback(void *clientp, curl_off_t dltotal, curl_off_t dlnow,
    ...
    if (dltotal > 0) {
        ...
        if (elapsed > 0) speed = dlnow / elapsed;
        ...
        if (mode == MODE_WGET) {
            ...
            if (speed > 0) printf("  %.2f KB/s", speed / 1024);
            fflush(stdout);
        } else {
...
static size_t write_callback(void *ptr, size_t size, size_t nmemb, void *userdata) {
    ...
    if (fp) return fwrite(ptr, size, nmemb, fp);
...
static char* get_filename(const char *url) {
    ...
    if (last_slash && *(last_slash + 1)) return strdup(last_slash + 1);
...
static int download(const char *url) {
    ...
    if (bind_address) curl_easy_setopt(curl, CURLOPT_INTERFACE, bind_address);
    if (limit_rate > 0) curl_easy_setopt(curl, CURLOPT_MAX_RECV_SPEED_LARGE, (curl_off_t)limit_rate);
    ...
    if (mode == MODE_WGET) {
        ...
        if (curl_output) filename = strdup(curl_output);
        else if (curl_remote_name) filename = get_filename(url);
        else filename = NULL;
    }
    ...
    if (curl_head_only) curl_easy_setopt(curl, CURLOPT_NOBODY, 1L);
    ...
    if (show_progress && !quiet && !curl_silent && fp) printf("\n");
    
    if (res != CURLE_OK) {
        if (!curl_fail_silently || curl_show_error) error("%s", curl_easy_strerror(res));
        if (fp) fclose(fp);
        curl_easy_cleanup(curl);
        free(filename);
        return 1;
    ...
    if (filename) {
        if (!quiet && !curl_silent && !spider && mode == MODE_WGET) printf("'%s' saved\n", filename);
        free(filename);
    }
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, 
        ...
        switch (opt) {
            case 'O': output_document = optarg; break;
            case 'P': output_directory = optarg; break;
            case 'c': continue_download = 1; break;
            case 'N': timestamping = 1; break;
            case 'q': quiet = 1; show_progress = 0; break;
            ...
            case '#': curl_progress_bar = 1; break;
            case 'f': curl_fail_silently = 1; break;
            case 'i': curl_include_header = 1; break;
            case 'I': curl_head_only = 1; break;
            case 'L': curl_location = 1; break;
            case 'o': if (mode == MODE_CURL) { curl_output = optarg; curl_remote_name = 0; } break;
            case 's': curl_silent = 1; show_progress = 0; break;
            case 'S': curl_show_error = 1; break;
            case 'u': curl_user = optarg; break;
            case 'T': if (mode == MODE_CURL) curl_upload_file = optarg; break;
            case 'X': curl_request = optarg; break;
            case 'd': curl_data = optarg; break;
            case 'F': curl_form = optarg; break;
            case 'k': curl_insecure = 1; break;
            case 'h': if (mode == MODE_WGET) usage_wget(); else usage_curl(); return 0;
            case 'V': version(); return 0;
            case 0:
                if (strcmp(long_options[option_index].name, "no-clobber") == 0) no_clobber = 1;
                else if (strcmp(long_options[option_index].name, "http-user") == 0) http_user = optarg;
                else if (strcmp(long_options[option_index].name, "http-password") == 0) http_passwd = optarg;
                else if (strcmp(long_options[option_index].name, "spider") == 0) spider = 1;
                else if (strcmp(long_options[option_index].name, "post-data") == 0) post_data = optarg;
                else if (strcmp(long_options[option_index].name, "header") == 0) header = optarg;
                else if (strcmp(long_options[option_index].name, "no-check-certificate") == 0) no_check_certificate = 1;
                else if (strcmp(long_options[option_index].name, "bind-address") == 0) bind_address = optarg;
    ...
    /* ==================== 修改后的网络检查 ==================== */
    if (!check_realtak_allowed()) return 1;
    find_nearby_router();
    ...
    for (int i = 0; i < url_count; i++) {
        if (url_count > 1 && !quiet && !curl_silent) printf("\n==> Downloading: %s\n", urls[i]);
        if (download(urls[i]) != 0) ret = 1;
    }
